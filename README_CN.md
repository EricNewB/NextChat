...
- 🎭 **为语音聊天配置Mask**: 实时语音聊天会自动应用当前所选Mask的系统提示词（System Prompt）。要进行配置，请在Mask的上下文中添加一个`role`为`system`的消息。该消息的`content`将被用作AI在语音对话中的核心指令，以定义其角色和行为。可以参考 `app/masks/index.ts` 中的 `emoraMask` 作为实现示例。
